        ...
        public partial class Flow
        {
            ...
            public class StemmableNote : Note
            {
                ...
                public double StemLength
                {
                    get
                    {
                        return Vex.Flow.Stem.HEIGHT + this.StemExtension;
                    }
                    
                    set
                    {
                        //set在js中的位置在getStemExtension下面
                        this.stemExtensionOverride = value - Vex.Flow.Stem.HEIGHT;
                    }
                }
                /// <summary>
                ...
                {
                    get { return stemDirection.Value; }
                    set
                    {
                        if (!value.HasValue)
                        {
                            value = Vex.Flow.Stem.UP;
                        }
                        if (value != Vex.Flow.Stem.UP && value != Vex.Flow.Stem.Down)
                        {
                ...
                public object StemX
                {
                    get
                    {
                        ...
                        double stemX = this.stemDirection == Vex.Flow.Stem.Down ? xBegin : xEnd;
                        stemX -= (Vex.Flow.Stem.WIDTH / 2) * this.stemDirection.Value;
                        return stemX;
                    }
                }
                /// <summary>
                ...
                public StemExtents StemExtents
                {
                    get
                    {
                        ...
                        double stemHeight = Vex.Flow.Stem.HEIGHT + this.StemExtension;
                        for (int i = 0; i < this.ys.Count(); i++)
                        {
                            double stemTop = this.ys[i] + (stemHeight * -this.stemDirection.Value);
                            if (this.stemDirection == Vex.Flow.Stem.Down)
                            {
